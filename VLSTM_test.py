...
def main():
    ...
    # Save directory
    save_directory = 'save_vlstm/' + str(sample_args.test_dataset) + '/'
    ...
    # Initialize net
    net = VLSTM(saved_args, True)
    net.cuda()

    # Get the checkpoint path
    checkpoint_path = os.path.join(save_directory, 'lstm_model_'+str(sample_args.epoch)+'.tar')
    ...
    for batch in range(dataloader.num_batches):
        ...
        # The sample function
        ret_nodes = sample(obs_nodes, obs_nodesPresent, sample_args, net, nodes, nodesPresent, saved_args, dimensions)
    ...
    with open(os.path.join(save_directory, 'results03_5.pkl'), 'wb') as f:
        pickle.dump(results, f)


def sample(nodes, nodesPresent, args, net, true_nodes, true_nodesPresent, saved_args, dimensions):
    '''
    The sample function
    params:
    ...
    for tstep in range(args.obs_length-1):
        # Do a forward prop
        out_obs, hidden_states, cell_states = net(nodes[tstep].view(1, numNodes, 2), [nodesPresent[tstep]], hidden_states, cell_states)
    ...
    for tstep in range(args.obs_length-1, args.pred_length + args.obs_length - 1):
        # Do a forward prop
        outputs, hidden_states, cell_states = net(ret_nodes[tstep].view(1, numNodes, 2),[nodesPresent[args.obs_length-1]], hidden_states, cell_states)
        ...
        next_x, next_y = sample_gaussian_2d(mux, muy, sx, sy, corr, nodesPresent[args.obs_length-1])
