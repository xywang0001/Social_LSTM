...
def train(args):
    ...
    NODES_INFO = Get_Nodes(args.batch_size, args.seq_length + 1)

    # Log directory
    log_directory = 'log_vlstm/'
    log_directory += str(args.leaveDataset) + '/'

    # Logging files
    log_file_curve_t = open(os.path.join(log_directory, 'log_curve_t.txt'), 'w')
    log_file_curve_v = open(os.path.join(log_directory, 'log_curve_v.txt'), 'w')
    log_file = open(os.path.join(log_directory, 'val.txt'), 'w')

    # Save directory
    save_directory = 'save_vlstm/'
    ...
    def checkpoint_path(x):
        return os.path.join(save_directory, 'lstm_model_'+str(x)+'.tar')
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in range(dataloader.num_batches):
            ...
            for sequence in range(dataloader.batch_size):
                ...
                nodes, nodesPresent = NODES_INFO.getSequence(sequence)
                ...
                # Forward prop
                outputs, _, _ = net(nodes[:-1],nodesPresent[:-1], hidden_states, cell_states)
        ...
        # Log loss values
        log_file_curve_t.write(str(epoch)+','+ str(loss_epoch)+',' + str(mse_loss_e) + '\n')
...
#--------------------------------------------------------------------------------------
        ...
        # Log loss values
        log_file_curve_v.write(str(epoch)+','+ str(loss_epoch)+','  + '\n')
    ...
    # Close logging files
    log_file.close()
    log_file_curve_t.close()
    log_file_curve_v.close()
